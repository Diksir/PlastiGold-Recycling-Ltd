...
const useMongoContent = Boolean(process.env.MONGODB_URI);
...
const mongoMediaCollection = process.env.MONGODB_MEDIA_COLLECTION || 'media_files';
...
async function mongoMediaStore() {
  ...
  const client = await globalCache.__plastigoldMongoClientPromise;
  return client.db(mongoDbName).collection(mongoMediaCollection);
}
...
export async function saveMediaFile(file, { allowedPrefixes = ['image/'], maxSizeMb = 5, label = 'file' } = {}) {
  ...
  if (useMongoContent) {
    const collection = await mongoMediaStore();
    await collection.updateOne(
      { _id: filename },
      {
        $set: {
...
export async function removeUploadedFileIfUnused(image) {
  if (useMongoContent && isUploadedFilePath(image)) {
    const collection = await mongoMediaStore();
    await collection.deleteOne({ _id: path.basename(image) }).catch(() => {});
    return;
  }
...
export async function readUploadedFile(filename) {
  if (useMongoContent) {
    const collection = await mongoMediaStore();
    const file = await collection.findOne({ _id: path.basename(filename) });
    if (!file?.bytes) {
      const error = new Error('Image not found.');
      error.code = 'ENOENT';
      throw error;
    }
    return Buffer.from(file.bytes.buffer || file.bytes);
  }
